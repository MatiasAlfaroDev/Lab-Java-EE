...
public class ClaveGrupoController {
    ...
    public Response distribuir(
        ...
        try {
            ...
            claveGrupoDAO.guardarOActualizarLote(chatId, req.envueltas, req.distribuidorId, req.version);

            // avisar a los miembros afectados para que invaliden su clave cacheada; sin esto,
            // un cliente ya abierto seguía cifrando/descifrando con la clave vieja para siempre
            // (podía enviar mensajes pero no leer los de nadie más, y viceversa)
            List<Integer> destinatarios = req.envueltas.stream().map(e -> e.miembroId).toList();
            String json = String.format("{\"type\":\"GROUP_KEY_ROTATED\",\"chatId\":%d}", chatId);
            ChatWebSocket.sendToUsers(destinatarios, json);
